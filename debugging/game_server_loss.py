...
SIMULATE_PACKET_LOSS_PERCENT = 13  
    ...
    while running and not game_over:
        ...
        with lock:
            ...
            for addr, client_state in clients.items():
                ...
                # Primary send (with optional simulated packet loss)
                if random.random() * 100 > SIMULATE_PACKET_LOSS_PERCENT:
                    server_utils.send_packet(sock, addr, MSG_SNAPSHOT, snapshot_id, payload, seq)
                    metrics.log_packet_sent()
                else:
                    # Simulated packet drop
                    if snapshot_id % 50 == 0:  # Log occasionally
                        log_message(f"[DEBUG] Simulated packet loss for snapshot {snapshot_id} to {addr}")
                ...
                if RETRANSMIT_K > 1:
                    ...
                    for old_sid in list(client_state.pending_snapshots)[-RETRANSMIT_K:-1]:
                        if old_sid > client_state.last_ack_snapshot:
                            # Resend this old snapshot (also subject to simulated loss)
                            if random.random() * 100 > SIMULATE_PACKET_LOSS_PERCENT:
...
def server_loop():
    ...
    if SIMULATE_PACKET_LOSS_PERCENT > 0:
        log_message(f"[DEBUG] ⚠️  SIMULATING {SIMULATE_PACKET_LOSS_PERCENT}% PACKET LOSS")
    log_message("")
