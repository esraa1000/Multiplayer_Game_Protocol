...
def analyze_scenario(csv_file, scenario_name):
    """Analyze metrics from a scenario and generate statistics"""
    ...
    # Aggregate all metrics across all clients
    all_latencies = []
    all_jitters = []
    all_errors = []
...
def generate_summary_table(all_stats):
    """Generate a summary table comparing all scenarios"""
...
def main():
    """Main function to generate statistics for all scenarios"""
    ...
    if all_stats:
        ...
        for stats in all_stats:
            ...
            if scenario == 'baseline':
                ...
                within_200 = stats['latency_p95'] <= 200
                print(f"  Latency 95th%: {stats['latency_p95']:.2f}ms (need ≤200ms for 99%) - "
                      f"{'✅ PASS' if within_200 else '❌ FAIL'}")
